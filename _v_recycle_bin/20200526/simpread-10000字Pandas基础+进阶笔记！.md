...
```
`# 通过传入一个序列给pd.Series初始化一个Series对象, 比如list``s1=pd.Series(list("1234"))``print(s1)``0    1``1    2``2    3``3    4``dtype:object`
```
...
```
`# 通过传入一个numpy的二维数组或者dict对象给pd.DataFrame初始化一个DataFrame对象``# 通过numpy二维数组``import numpy as np``df1 = pd.DataFrame(np.random.randn(6,4))``print(df1)` `0   1   2   3``0   -0.646340   -1.249943   0.393323    -1.561873``1   0.371630    0.069426    1.693097    0.907419``2   -0.328575   -0.256765   0.693798    -0.787343``3   1.875764    -0.416275   -1.028718   0.158259``4   1.644791    -1.321506   -0.33742``5   0.8206895   0.006391    -1.447894   0.506203    0.977295``# 通过dict字典``df2 = pd.DataFrame({ 'A' : 1.,` `'B' : pd.Timestamp('20130102'),``                     'C' :pd.Series(1,index=list(range(4)),dtype='float32'), ``                     'D' : np.array([3] * 4,dtype='int32'),                                          ``                     'E' : pd.Categorical(["test","train","test","train"]),                     ``                     'F' : 'foo' })``print(df2)` `A   B   C   D   E   F``0   1.0 2013-01-02  1.0 3   test    foo``1   1.0 2013-01-02  1.0 3   train   foo``2   1.0 2013-01-02  1.0 3   test    foo``3   1.0 2013-01-02  1.0 3   train   foo`
```
...
```
`# 打印对象的时候，第一列就是索引``print(s1)``0    1``1    2``2    3``3    4``dtype: object``# 或者只查看索引, DataFrame同理``print(s1.index)`
```
...
这里的增删查改主要**基于DataFrame对象，**为了有足够数据用于展示，这里选择**tushare**的数据。  
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/RAwjyr0lc0KlXWp5qZETdGibrDuBUYgPl9tFz8l3iba7IH5hZbSE5QYh8juZjvtgLZvlmfdx04xj8AUEZh5GlA5Q/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

  

![](https://mmbiz.qpic.cn/mmbiz_jpg/RAwjyr0lc0KlXWp5qZETdGibrDuBUYgPlVRBEQ380aplEhibDWzIqHF00cD785pkdaQQmeIMZTPg9e3tzBAL9x0g/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
`# 查看df数据类型``df.dtypes``date       object``open        float64``close        float64``high         float64``low          float64``volume    float64``code       object``dtype: object`
```
...
```
`# float数据类型以{:.4f}格式显示，即显示完整数据且保留后四位``pd.options.display.float_format = '{:.4f}'.format`
```
...
```
`# 删除含有NaN的任意行``df.dropna(how='any')``# 删除含有NaN的任意列``df.dropna(how='any', axis=1)``# 将NaN的值改为5``df.fillna(value=5)`
```
...
```
`# 这里使用notbook，为了直接在输出中显示，需要以下配置``%matplotlib inline``# 绘制Open,Low,Close.High的线性图``df[["Open", "Low", "High", "Close"]].plot()``# 绘制面积图``df[["Open", "Low", "High", "Close"]].plot(kind="area")`
```
...
```
`# 查看数据类型``log_df.dtypes`  `Host                  object``Time          datetime64[ns]``Method                object``Path                  object``Protocol              object``status                object``size                  object``User_Agent            object``dtype: object`
```
...
```
`# 统计不同status值的次数``log_df.Status.value_counts()``200    5737``304    1540``404    1186` `400     251``302      37``403       3``206       2``Name: Status, dtype: int64`
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/yAyQKzCbAHbb9bxcpe31A69ZFv8zEg4H1DiavO6eLexSDeYl9V8sVGPQOdzvYfbCxXWEwR6LzM5G4P46ZRB254w/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)  
...
```
`log_df2 = log_df.copy()``# 为每行加一个request字段，值为1``log_df2["Request"] = 1``# 每一小时统计一次request数量，并将NaN值替代为0，最后绘制线性图，尺寸为16x9``log_df2.Request.resample("H").sum().fillna(0).plot(kind="line",figsize=(16,10))`
```

  

![](https://mmbiz.qpic.cn/mmbiz_png/RAwjyr0lc0KlXWp5qZETdGibrDuBUYgPlc8aO6qmdHFib3xFC7K3LFDABmDDKia9qiaU1IoeQvHZbL8MgCh9DGomUQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
`分别对202，304，404状态重新取样，并放在一个列表里面``req_df_lis = [``log_df2[log_df2.Status == 200].Request.resample("H").sum().fillna(0),` `log_df2[log_df2.Status == 304].Request.resample("H").sum().fillna(0),` `log_df2[log_df2.Status == 404].Request.resample("H").sum().fillna(0)` `]``# 将三个dataframe组合起来``req_df = pd.concat(req_df_lis,axis=1)``req_df.columns = ["200", "304", "404"]``# 绘图``req_df.plot(figsize=(16,10))`
```

  

![](https://mmbiz.qpic.cn/mmbiz_png/RAwjyr0lc0KlXWp5qZETdGibrDuBUYgPlyRB1AXic8U6eYibpLFExt67nxNicyTpKDsGGQItfTgliaAia0G2gEQ9N7nQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![](https://mmbiz.qpic.cn/mmbiz/hN1l83J6Ph8RUn1vxnF12vxHSpKB78B8kfjQ3jzCfYTC84EZUsiaQyObtjmolNqM63EXS0jWAr9F2y6mVP80zDw/640?wx_fmt=other&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)